...
const AddInstructorSchedule = ({
  ...
  const [isScheduleForBothGroups, setIsScheduleForBothGroups] = useState("");
  const [unpublishedSchedules, setUnpublishedSchedules] = useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [scheduleRes, sectionRes, subjectRes, roomRes, departmentRes] =
        await Promise.all([
      ...
      .filter((schedule) =>
        dept === "CICS"
          ? schedule.subject === data.subject &&
            ...
            schedule.section_name == data.section &&
            schedule.section_group === data.group
          : schedule.subject === data.subject &&
            (schedule.class_type === data.course_type ||
              schedule.class_type !== data.course_type) &&
            schedule.section_name === data.section &&
    ...
    days.forEach((day) => {
      ...
      for (let hour = 7; hour <= 20 - duration; hour++) {
        ...
        const alternateGroupAvailable =
          (subject && subject.subject_type === "Minor") ||
          isScheduleForBothGroups === "Both"
            ? !schedules.some(
                (schedule) =>
                  schedule.section_name === data.section &&
                  schedule.section_group ===
                    (data.group === "Group 1" ? "Group 2" : "Group 1") &&
                  schedule.day === day &&
      ...
      try {
        ...
        if (
          ...
          const sectionHasOtherGroups = sections.some(
            (sec) =>
              sec.section_name === data.section &&
              sec.section_group !== data.group
          );
          ...
          if (sectionHasOtherGroups) {
            ...
            if (
              ...
              await axios.post(`${url}api/activity/adding`, {
                ...
                action: "Add",
                details: `${data.section} - ${alternateGroup}`,
                type: "schedule",
              });
            }
          }
        ...
        await axios.post(`${url}api/activity/adding`, {
          ...
          department_code: currentDepartment,
          action: "Add",
          details: `${data.section} - ${data.group}`,
          type: "schedule",
        });
  ...
  const uniqueSections = Array.from(
    ...
    .map((section_name) => {
      return sections.find((s) => s.section_name === section_name);
    })
  ...
  const filteredSections = uniqueSections.filter((section) => {
    ...
    const matchesSearch =
      section.section_name
        .toLowerCase()
        .includes(searchSectionQuery.toLowerCase()) ||
            ...
            >
              <div className="flex flex-col justify-between h-full">
                ...
                <div className="flex gap-4">
                  ...
                  >
                    <label className="w-1/2">Section</label>
                    <input
                      type="text"
                      name="SectionName"
                    ...
                    />
                    {
                      //check if the data.section has a group in sections
                      sections.some(
                        (section) =>
                          section.section_name === data.section &&
                          section.section_group
                      ) ? (
                        <select
                          value={
                          ...
                          onChange={(e) => {
                            if (e.target.value === "Both") {
                              setData({ ...data, group: "Group 1" });
                              setIsScheduleForBothGroups("Both");
                            } else {
                              setIsScheduleForBothGroups("");
                          ...
                          className={`${
                            errors.time_error
                              ? "border border-red-500"
                              : "border border-gray-300"
                          } ml-1 w-1/2 border border-gray-300 h-8 rounded-md text-sm focus:outline focus:outline-green-400 text-black`}
                        >
                          <option value="">Group</option>
                          {sections
                            .filter(
                              (section) => section.section_name === data.section
                            )
                            .map((section, index) => (
                          ...
                          {sections.filter(
                            (section) =>
                              section.section_name === data.section &&
                              section.section_group === "Group 1"
                          ).length > 0 &&
                          sections.filter(
                            (section) =>
                              section.section_name === data.section &&
                              section.section_group === "Group 2"
                          ).length > 0 ? (
                    ...
                    {errors.subject_error && (
                      <p className="text-[0.8rem] flex items-center px-2 py-[0.35rem] rounded-md border border-red-400">
                        <FontAwesomeIcon
                          ...
                          className="text-orange-500 text-lg mr-2"
                        />
                        Subject has reached meeting quota for this section.
                      </p>
                    )}
                  </div>
                  {errors.subject_error && (
                      ...
                      {errors.course_error && (
                        <p className="text-[0.8rem] flex items-center px-2 py-[0.35rem] rounded-md border border-red-400">
                          <FontAwesomeIcon
                            ...
                            className="text-orange-500 text-lg mr-2"
                          />
                          Reached meeting quota for this section.
                        </p>
                      )}
                    </div>
                    {errors.course_error && (
                    ...
                    <select
                      ...
                      className={`${
                        errors.time_error
                          ? "border border-red-500"
                          : "border border-gray-300"
                      } w-full h-8 rounded-md px-2 text-sm focus:outline focus:outline-green-400 text-black border border-gray-300`}
                      onChange={(e) =>
                    ...
                    {errors.time_error && (
                      <p className="text-[0.8rem] flex items-center px-2 py-[0.35rem] rounded-md border border-red-400">
                        <FontAwesomeIcon
                          ...
                          className="text-orange-500 text-lg mr-2"
                        />
                        Section not available detected between{" "}
                        {`${
                          parseInt(
        ...
        <div className="flex lg:flex-row flex-col justify-between lg:gap-8 gap-4 h-1/2 w-full">
          <div className="flex flex-col justify-between items-center h-full lg:w-[33.3%] w-full lg:border-none border border-gray-300 rounded-md p-4">
            ...
            <ul className="flex flex-col py-2 gap-1 w-full h-full">
              {filteredSections
                .slice(
                  currentSectionPage * itemsPerPage,
                  (currentSectionPage + 1) * itemsPerPage
                )
                .map((section, index) => (
                  <li
                    key={index}
            ...
            <ReactPaginate
              ...
              pageCount={Math.ceil(filteredSections.length / itemsPerPage)}
              onPageChange={({ selected }) => {
                setCurrentSectionPage(selected);
              }}
...
export default AddInstructorSchedule;
